...
import { Search, BookPlus, X, ArrowLeft } from "lucide-react";
...
export default function BookTopBar({ routeName = "books.index" }) {
  const { filters } = usePage().props; // ✅ get filters from server
  ...
  const handleSearch = (e) => {
    ...
    router.get(
      route(routeName),
      { ...filters, search }, // keep filter + search together
  ...
  const handleFilter = (value) => {
    router.get(
      route(routeName),
      { ...filters, status: value }, // ✅ sync filter with search
      { preserveState: true, replace: true }
    );
  };
